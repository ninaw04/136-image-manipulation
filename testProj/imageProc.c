...
// Averages corresponding values in two matrices, returns a matrix
// Trusts caller to ensure that matrices are the same height and width
Matrix averageTwoMatrices(Matrix m1, Matrix m2) {
    Matrix avgMat = createMatrix(m1.height, m1.width);
    
    for (int y = 0; y < m1.height; y++) {
        for (int x = 0; x < m1.width; x++) {
            double avg = (m1.map[y][x] + m2.map[y][x]) / 2;
            avgMat.map[y][x] = avg;
        }
    }
    return avgMat;
